...
namespace nsBalloon {
    ...
    bool GameScene::Start()
    {
        ...
        m_gameSceneState = nsGSConstant::GAME_STATE_LOOK_STAGE;
    ...
    void GameScene::Update()
    {
        ...
        {
        case nsGSConstant::GAME_STATE_LOOK_STAGE:
            LookStage();
            break;

        case nsGSConstant::GAME_STATE_START_CALL:
            GameStartCall();
            break;

        case nsGSConstant::GAME_STATE_BATTLE:
            Battle();
            m_isResult = false;
            break;
        case nsGSConstant::GAME_STATE_RESULT:
            if (m_isResult == false) {
                for (int i = 0; i < 4; i++)
                {
    ...
    {
        //最初の処理
        if (m_lookStageTimer == nsGSConstant::INI_LOOK_STAGE_TIME)
        {
        ...
        if (g_pad[0]->IsTrigger(enButtonA) || m_lookStageTimer <= 0)
        {
            ...
            m_gameSceneState = nsGSConstant::GAME_STATE_START_CALL;
            g_camera3D->SetPosition(nsGSConstant::INI_CAMERA_POS);
            g_camera3D->SetTarget(nsGSConstant::INI_CAMERA_TARGET_POS);
        }
        m_lookStageTimer--;
    }
    ...
    void GameScene::GameStartCall()
    {
        ...
        if (m_gameStartCallTimer == nsGSConstant::INI_GAME_START_CALL_TIME)
        {
        ...
        else if (m_gameStartCallTimer <= 0)
        {
            ...
            m_gameSceneState = nsGSConstant::GAME_STATE_BATTLE;
    ...
    void GameScene::Battle()
    {
        ...
        if (m_alivePlCount == 0)
        {
            SetGameState(nsGSConstant::GAME_STATE_RESULT);
    ...
    void GameScene::SetIniPosition()
    {
        ...
        m_gameStartCallTimer = nsGSConstant::INI_GAME_START_CALL_TIME;
        m_lookStageTimer = nsGSConstant::INI_LOOK_STAGE_TIME;
