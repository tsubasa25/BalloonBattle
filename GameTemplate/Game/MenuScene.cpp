...
namespace nsBalloon {
	...
	bool MenuScene::Start()
	{
		...
		m_plCountFontRender->SetPosition(nsMSConstant::PLYAER_COUNT_FONT_POS);
		m_plCountFontRender->SetText(L"プレイ人数 : " + std::to_wstring(m_playerCount) + L"人");

		m_stageSelectFontRender = NewGO<FontRender>(0);
		m_stageSelectFontRender->SetPosition(nsMSConstant::STAGE_NUM_FONT_POS);
		...
		m_menuBGM->SetVolume(nsMSConstant::SOUND_MENU_BGM_VOLUME);
		m_menuBGM->Play(true);
	...
	void MenuScene::AddSelectNum(int num)
	{
		...
		{
		case 0:
			m_cursorPos.y = nsMSConstant::PLYAER_COUNT_FONT_POS.y;
			break;
		case 1:
			m_cursorPos.y = nsMSConstant::STAGE_NUM_FONT_POS.y;
			break;
		}
		m_cursorFontRender->SetPosition(m_cursorPos);
	}
	...
	void MenuScene::SetStage()
	{
		...
		//ステージ名を表示
		const std::wstring stageName[3] = { {L"SkyIslandStage"},{L"BuildingStage"},{L"TrapStage"} };

		m_stageSelectFontRender->SetText(stageName[m_stageNum]);
