...
namespace nsBalloon {
	...
	bool GimmickNeedle::Start()
	{
		...
		//針は、板より下の位置で引っ込んでいる。
		m_needlePos = { m_needleBoardPos.x, m_needleBoardPos.y - nsGNConstant::NEEDLE_HEIGHT, m_needleBoardPos.z };
	...
	void GimmickNeedle::Update()
	{
		...
		if (m_isNeedleUp == false)
		{
			...
			if (m_needleMoveTimer >= nsGNConstant::NEEDLE_DOWN_INTERVAL)
			{
				m_isNeedleUp = true;
				m_needleMoveTimer = 0;
			}
		...
		else
		{
			...
			QueryGOs<Player>("player", [this](Player* player)->bool {
				...
				if (player->GetCanMove() == true)
				{
					...
					if (fabsf(m_diff.x) < nsGNConstant::NEEDLE_BOARD_HALF_SCALE
						&& fabsf(m_diff.z) < nsGNConstant::NEEDLE_BOARD_HALF_SCALE
						&& m_needlePos.y + nsGNConstant::NEEDLE_HEIGHT > plPos.y	//プレイヤーの位置が針の高さより低いかどうか
						)
					{
						player->BreakBalloon();
						player->PlayerDeath();
			...
			if (m_needleMoveTimer > nsGNConstant::NEEDLE_UP_INTERVAL)
			{
				m_isNeedleUp = false;
				m_needleMoveTimer = 0;
			}
	...
	void GimmickNeedle::Move()
	{
		if (m_isNeedleUp == true)
		{
			...
			if (m_needleMoveTimer < nsGNConstant::NEEDLE_UP_INTERVAL - (nsGNConstant::NEEDLE_UP_INTERVAL - 10))
			{
				m_needlePos.y += 20.0f;
			...
			else if (m_needleMoveTimer > nsGNConstant::NEEDLE_UP_INTERVAL - 10)
			{
				m_needlePos.y -= 20.0f;
				if (m_needlePos.y < m_needleBoardPos.y - nsGNConstant::NEEDLE_HEIGHT)
					m_needlePos.y = m_needleBoardPos.y - nsGNConstant::NEEDLE_HEIGHT;
