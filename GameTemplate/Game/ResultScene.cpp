...
namespace nsBalloon {
	...
	bool ResultScene::Start()
	{
		...
		m_gameSetFontRender->SetPosition({ nsRSConstant::GAMESET_FONT_POSITION });
		m_gameSetFontRender->SetScale(nsRSConstant::GAMESET_FONT_SCALE);
		m_gameSetFontRender->SetShadowFlag(true);
		m_gameSetFontRender->SetShadowOffset(nsRSConstant::GAMESET_FONT_OFFSET);
	...
	void ResultScene::GameSet()
	{
		...
		else
		{
			if (m_winnerPl != nullptr)//勝者がいるとき
			{
				...
				m_winFontRender->SetScale(nsRSConstant::GAMESET_FONT_SCALE);
				m_winFontRender->SetText(L"PLAYER " + std::to_wstring(m_winnerPl->GetPlayerNum() + 1) + L" WIN!!");
				m_winFontRender->SetPosition({ nsRSConstant::WIN_FONT_POSITION });
				m_winFontRender->SetShadowFlag(true);
				m_winFontRender->SetShadowOffset(nsRSConstant::GAMESET_FONT_OFFSET);
			...
			else//同時に死んだとき
			{
				...
				m_winFontRender->SetPosition({ nsRSConstant::DRAW_FONT_POSITION });
				m_winFontRender->SetShadowFlag(true);
				m_winFontRender->SetShadowOffset(nsRSConstant::GAMESET_FONT_OFFSET);
	...
	void ResultScene::ZoomWinner()
	{
		if (m_resultBGMFlg == false)//リザルトBGMを流す
		{
			...
			m_resultBGM->SetVolume(nsRSConstant::SOUND_RESULT_BGM_VOLUME);
			m_resultBGM->Play(true);
			g_camera3D->SetTarget({ nsRSConstant::CAMERA_TARGET_POSITION });
		}

		if (m_winDisplayedTime > 0)//WINが表示されている間
		{
			m_winDisplayedTime--;
			m_cameraMoveSpeed = m_cameraMoveSpeed * nsRSConstant::VALUE_MULTIPLY_IN_SPEED;//指数関数的にカメラが近寄っていく
			m_cameraPos.z += m_cameraMoveSpeed;

			if (m_cameraPos.z > nsRSConstant::CAMERA_SHORTEST_RANGE)//近づきすぎないようにする
			{
				m_cameraPos.z = nsRSConstant::CAMERA_SHORTEST_RANGE;//固定する
			}
		...
		else if (g_pad[0]->IsTrigger(enButtonA) || m_winDisplayedTime <= 0)//Aボタンを押すか、時間が立つか
		{
			...
			m_cursorSprite->Init("Assets/Image/Cursor.DDS", nsRSConstant::CURSOR_SPRITE_WIDTH, nsRSConstant::CURSOR_SPRITE_HEIGHT);
			m_cursorSprite->SetPosition(nsRSConstant::RESULT_CURSOR_FONT_INI_POS);
			m_cursorSprite->SetScale({ Vector3::One });

			m_retriFontRender = NewGO<FontRender>(1);
			m_retriFontRender->SetPosition(nsRSConstant::RETRI_FONT_POS);
			...
			m_backMenuFontRender->SetPosition(nsRSConstant::BACK_MENU_FONT_POS);
			...
			m_backTitleFontRender->SetPosition(nsRSConstant::BACK_TITLE_FONT_POS);
	...
	void ResultScene::TimeUp()
	{
		...
		else//タイムアップした
		{
			...
			QueryGOs<Player>("player", [this](Player* player)->bool {
				...
				plFont->SetScale(nsRSConstant::PLAYER_FONT_SCALE);
				plFont->SetShadowFlag(true);
				plFont->SetShadowOffset(nsRSConstant::GAMESET_FONT_OFFSET);
	...
	void ResultScene::Menu()
	{
		...
		if (g_pad[0]->IsTrigger(enButtonA))//Aを押せば
		{
			...
			m_resultSE->SetVolume(nsRSConstant::SOUND_RESULT_SE_VOLUME);
			...
			if (m_selectMenu == enRetri)//リトライを押せば
			{
				...
				m_gameScene->SetGameState(nsGSConstant::GAME_STATE_LOOK_STAGE);//ゲームシーンの状態を更新する::ステージを見渡す
	...
	void ResultScene::SetCursorPos()
	{
		...
		if (m_cursorPos.x > nsRSConstant::CURSOR_FONT_MAX_POS_X)	//カーソルの動きをリセットする。
		{
			m_cursorMoveSpeedX = nsRSConstant::INI_CURSOR_MOVE_SPEED_X;
			m_cursorPos.x = nsRSConstant::RESULT_CURSOR_FONT_INI_POS.x;
		}
	...
	void ResultScene::isSelectMenu(bool isUp)
	{
		...
		{
		case enRetri:
			m_cursorPos.y = nsRSConstant::RETRI_FONT_POS.y - 20;
			break;
		case enBackMenu:
			m_cursorPos.y = nsRSConstant::BACK_MENU_FONT_POS.y - 20;
			break;
		case enBackTitle:
			m_cursorPos.y = nsRSConstant::BACK_TITLE_FONT_POS.y - 20;
			break;
		}
	}
