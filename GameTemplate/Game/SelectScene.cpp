...
namespace nsBalloon {
	...
	bool SelectScene::Start()
	{
		...
		m_selectBGM->SetVolume(nsSSConstant::SOUND_SELECT_BGM_VOLUME);
	...
	{
		if ((SelectScene::m_titleState == TitleState::enBackTitle))
		{
	...
	void SelectScene::SelectionPlayerCount()
	{
		if (g_pad[0]->IsTrigger(enButtonB))
		{
			m_cancelSE = NewGO<SoundSource>(0);
			m_cancelSE->Init(L"Assets/sound/キャンセル音.wav");
			m_cancelSE->SetVolume(nsSSConstant::SOUND_SELECT_SE_VOLUME);
		...
		if (g_pad[0]->IsTrigger(enButtonA))
		{
			...
			m_selectSE->SetVolume(nsSSConstant::SOUND_SELECT_SE_VOLUME);
		...
		{
			char filePath[256];
			sprintf(filePath, "Assets/Image/c%d.dds", m_playerCount);
			m_cPlayerCount_Sprite->Init(filePath, 70, 140);
	...
	void SelectScene::SelectionStockNumber()
	{
		if (g_pad[0]->IsTrigger(enButtonB))
		{
			m_cancelSE = NewGO<SoundSource>(0);
	...
	void SelectScene::SelectionStage()
	{
		if (g_pad[0]->IsTrigger(enButtonB))
		{
			m_cancelSE = NewGO<SoundSource>(0);
		...
		if (m_selectNum[0][0] == true)
		{
			...
			m_stageBalloonPos[1].y = nsSSConstant::BUIL_BALL_POS.y + (sin(m_selectLoopCount)) * 10.0f;
		}
		else if (m_selectNum[0][1] == true)
		{
			m_stageNum = 1;
			m_selectLoopCount += 0.05f;
			m_stageBalloonPos[0].y = nsSSConstant::BUIL_BALL_POS.y + (sin(m_selectLoopCount)) * 10.0f;
		}
		else if (m_selectNum[1][0] == true)
		{
			m_stageNum = 2;
			m_selectLoopCount += 0.05f;
			m_stageBalloonPos[2].y = nsSSConstant::TRAP_BALL_POS.y + (sin(m_selectLoopCount)) * 10.0f;
		}
		else if (m_selectNum[1][1] == true)
		{
			m_stageNum = 3;
			m_selectLoopCount += 0.05f;
			m_stageBalloonPos[3].y = nsSSConstant::TRAP_BALL_POS.y + (sin(m_selectLoopCount)) * 10.0f;
		}
		for (int i = 0; i < 4; i++)
		{
			m_stageBalloon_Sprite[i]->SetPosition(m_stageBalloonPos[i]);
